import { firstDayOfMonth } from './date-utils'
import dayjs from './dayjs'
import { Coords, Weather, FormattedWeather, HistWeatherData } from './weather-types.ts'
...
const getCoords = async (zip: number): Promise<CoordData> => {
 const res = await fetch(`https://geocode.maps.co/search?q=${zip}&api_key=${process.env.EXPO_PUBLIC_GEOCODE_API_KEY}`)
